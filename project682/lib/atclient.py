...
class atClient:
    ...
    def connect(self, atserver, atport, timeout=5.0):
        ...
        try:
            ...
            log.exception(err, "During atSign server {}:{} cnx", atserver, atport)
            #self.sock.close()
            self.sock = None
            raise err
    ...
    def authenticate(self, pkamKey):
        ...
        try:
            ...
            log.exception(err, "during atsign server {}:{} cnx", self.server, self.port)
            #self.sock.close()
            self.sock = None
            raise err
    ...
    def getsharedkey(self, privKey):
        ...
        try:
            ...
            if response.startswith('data:'):
                ...
                self.sharedkey=ubinascii.a2b_base64.toString(shared_key)
    ...
    def attalk(self, msg:bytes, topic:str="attalk", namespace:str="ai6bh"):
        ...
        try:
            ...
            log.exception(err, "during info publish to atsign server {}:{} cnx", self.server, self.port)
            #self.disconnect()